...
Sub DataCleanup()
    ...
    PopulateChart dataWs, ws2, 23, "Supplier Anomaly Count Per Defect Code", suppliers(1), lastRow, True
    Set ws3 = BuildScaffold(dataWs, S3, "Project Name", "RefreshSheet3", suppliers, supplierCount, "Supplier:")
    PopulateChart dataWs, ws3, projShortCol, "Anomaly Count Per Project", suppliers(1), lastRow
...
Sub RefreshSheet2()
    ...
    If selSup = "" Then MsgBox "Select a supplier in B1.", vbExclamation: Exit Sub
    lastRow = dataWs.Cells(dataWs.Rows.Count, "A").End(xlUp).Row
    PopulateChart dataWs, aws, 23, "Supplier Anomaly Count Per Defect Code", selSup, lastRow, True
End Sub
Sub RefreshSheet3()
    Dim dataWs As Worksheet
    Dim aws As Worksheet
