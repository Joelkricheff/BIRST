...
Sub DataCleanup()
    ...
    s1Name = "Supplier Anomaly Count Per Disposition"
    ...
    For i = lastRow To 4 Step -1
        If Trim(dataWs.Cells(i, "H").Value) <> "CLOSED" Then
            dataWs.Rows(i).Delete
        End If
    Next i
    lastRow = dataWs.Cells(dataWs.Rows.Count, "U").End(xlUp).Row
    For i = lastRow To 4 Step -1
        If Left(Trim(CStr(dataWs.Cells(i, "U").Value)), 1) <> "4" Then
            dataWs.Rows(i).Delete
        End If
    Next i
    ...
    For i = lastRow To 4 Step -1
        If UCase(Left(Trim(dataWs.Cells(i, 23).Value), 9)) = "NO DEFECT" Then
            dataWs.Rows(i).Delete
        End If
    Next i
    lastRow = dataWs.Cells(dataWs.Rows.Count, 24).End(xlUp).Row
    For i = lastRow To 4 Step -1
        If UCase(Trim(dataWs.Cells(i, 24).Value)) = "PROMOTE TO HAR" Then
            dataWs.Rows(i).Delete
        End If
    Next i
    lastRow = dataWs.Cells(dataWs.Rows.Count, 24).End(xlUp).Row
    For i = lastRow To 4 Step -1
        If UCase(Trim(dataWs.Cells(i, 24).Value)) = "NDF" Then
            dataWs.Rows(i).Delete
        End If
    Next i
    ...
    selSup = suppliers(1)
    For i = 4 To lastRow
        If Trim(dataWs.Cells(i, "A").Value) = selSup Then
            dVal = Trim(dataWs.Cells(i, 23).Value)
            If dVal <> "" Then
                found = False
                For j = 1 To d1Count
    ...
    For i = 4 To lastRow
        If Trim(dataWs.Cells(i, "A").Value) = selSup Then
            dVal = Trim(dataWs.Cells(i, 24).Value)
            If dVal <> "" Then
                found = False
                For j = 1 To d2Count
...
Sub RefreshSheet1()
    ...
    Dim dVal As String
    Dim found As Boolean
    sheetName = "Supplier Anomaly Count Per Disposition"
    For Each ws In ThisWorkbook.Worksheets
        If ws.Name = sheetName Then Set analysisWs = ws
        If ws.Name <> sheetName And ws.Name <> "Defect Code Analysis" Then
    ...
    If analysisWs Is Nothing Or dataWs Is Nothing Then
        ...
        Exit Sub
    End If
    selSup = Trim(analysisWs.Cells(1, 2).Value)
    If selSup = "" Then
