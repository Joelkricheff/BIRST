...
Public Const PROJNUMCOL As Long = 36
Sub DataCleanup()
    ...
    If lastRow >= 4 Then
        dataWs.Range(dataWs.Cells(4, projNameCol), dataWs.Cells(lastRow, projNameCol)).Formula = _
            "=IFERROR(VLOOKUP(LEFT(" & cl & "4,5),'Project Key'!A:B,2,FALSE),IFERROR(VLOOKUP(VALUE(LEFT(" & cl & "4,5)),'Project Key'!A:B,2,FALSE),""""))"
    End If
    Application.Calculate
...
Sub PopulateChart(dataWs As Worksheet, analysisWs As Worksheet, countCol As Long, chartTitle As String, selSup As String, lastRow As Long)
    ...
    For i = 4 To lastRow
        If SafeText(dataWs.Cells(i, "A")) = selSup Then
            dVal = SafeText(dataWs.Cells(i, countCol))
            If dVal <> "" Then
                found = False
                For j = 1 To n
                    If codes(j) = dVal Then counts(j) = counts(j) + 1: found = True: Exit For
                Next j
                If Not found Then
                    n = n + 1
